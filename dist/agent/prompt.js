...
function loadFileContent(path, label) {
    try {
...
function buildSkillSection(skillContent) {
    return [
        '---',
        '',
        '# Project Harness',
...
function buildDiffSection(diffSection) {
    if (diffSection.mode === 'empty' && !diffSection.content) {
        return [];
    }
    const modeLabel = diffSection.mode === 'full'
...
export function buildExecutionContext(config, context) {
    ...
    const diffScopeLabel = context.gitContext.includeUncommitted
        ...
        : diffRange;
    const largePrNote = context.diffStats.fileCount > 20
    ...
    lines.push('', '## Execution Context', '', `\`cwd\` = \`${config.repoRoot}\`. Diff and rules are already embedded below; use tools to expand context in Phase 2.`, '');
    if (config.pullRequestId > 0) {
        lines.push(`- **Pull Request ID (Azure DevOps):** #${config.pullRequestId}`, `- **PR ID Source:** \`${config.pullRequestIdSource || 'unknown'}\``, `- **Warning:** do not confuse the PR ID with linked Work Item IDs (User Story/Task).`, '');
    }
    lines.push(`- **Branch:** \`${sourceRef}\` → \`${targetRef}\``, `- **Diff range:** \`${diffScopeLabel}\``, `- **Stack:** \`${config.stack}\``, `- **Minimum score for threads (\`AGENTIC_CODE_REVIEWERS_SCORE_MIN\`):** **${config.scoreMin}** — include in \`reviews\` **only** findings with \`score ≥ ${config.scoreMin}\`; the runner discards the rest before creating threads (pipeline default: 6; CLI \`--score-min\` and env take precedence).`, `- **Eligible files:** ${context.diffStats.fileCount}`, context.diffStats.files.length > 0
        ? `- **List:** ${context.diffStats.files.slice(0, 30).join(', ')}${context.diffStats.files.length > 30 ? '...' : ''}`
        : '', `- **Include:** ${config.includePatterns.join(', ')}`, `- **Exclude:** ${config.excludePatterns.join(', ')}`, '');
    if (context.rulesContext) {
        lines.push(context.rulesContext, '');
    }
    return lines;
}
function buildSeedTestSection() {
    return [
...
function buildTwoPhaseWorkflow(context, scoreMin) {
    const diffRange = context.gitContext.diffRange;
    ...
    const omittedNote = context.diffSection.omittedFiles > 0
...
function buildVerdictAndAdoPolicy(scoreMin) {
    return [
        '',
        '### Final Verdict',
        '',
...
export function buildAgentPrompt(config, context) {
    ...
    if (stackPromptContent) {
        sections.push('---', '', `# Specific Stack Recommendations (${config.stack})`, '', stackPromptContent, '');
    }
    ...
    if (moduleContents.length > 0) {
        sections.push('---', '', '# Change-Type Directives', '', moduleContents.join('\n\n---\n\n'), '');
    }
    ...
    sections.push(...buildExecutionContext(config, context), ...buildDiffSection(context.diffSection));
