...
import { getCodeReviewPostingPlan, getNewReviewsFromPlan, parseCodeReviewResponse, shouldPostReviewSummary, simulateThreadResolution, } from './ado/post-comments.js';
import { buildSafeOutputOptions } from './ado/safe-outputs.js';
import { filterGatePendingThreads } from './ado/review-context.js';
import { decideRoundEscalation, splitReviewsForEscalation, } from './ado/round-state.js';
...
import { formatDiffSizeKb, getDiffBreakdown, getDiffFileSummaries, getDiffPatch, prepareLocalReviewWorkspace, } from './git/diff.js';
...
function logDiffFileSummaries(logger, title, summaries) {
...
async function main() {
    ...
    const stackSourceLabel = config.stackSource === 'cli'
        ? 'configurada via CLI'
    ...
    if (config.pullRequestId) {
        logger.info(`Pull Request ID: #${config.pullRequestId}` +
            (config.pullRequestIdSource ? ` (fonte: ${config.pullRequestIdSource})` : ''));
        logger.info(`ADO Context: org=${config.organization}, project=${config.project}, repo=${config.repositoryName}`);
    }
    logger.section('Preparando repositório local');
    const gitContext = prepareLocalReviewWorkspace(config.repoRoot, config.sourceBranch, config.targetBranch, (msg) => logger.info(msg));
    gitContext.includeUncommitted = config.includeUncommitted;
    const diffOptions = { includeUncommitted: config.includeUncommitted };
    const diffStats = getDiffBreakdown(config.repoRoot, gitContext.diffRange, config.includePatterns, config.excludePatterns, diffOptions);
    logger.section('Git diff — comparação de branches');
    logger.info(`Range: ${gitContext.diffRange}${config.includeUncommitted ? ' + working tree (uncommitted)' : ''} (${config.sourceBranch} → ${config.targetBranch})`);
    const filteredDiffOptions = { ...diffOptions, files: diffStats.filteredFiles };
    const diffFileSummaries = getDiffFileSummaries(config.repoRoot, gitContext.diffRange, filteredDiffOptions);
    ...
    const diffSection = buildDiffPromptSection(config.repoRoot, gitContext.diffRange, diffStats.filteredFiles, diffOptions, config.diffMaxBytes);
    const scopedDiffText = getDiffPatch(config.repoRoot, gitContext.diffRange, filteredDiffOptions);
    const safeOutputOptions = buildSafeOutputOptions(config, scopedDiffText);
    if (diffSection.mode !== 'empty') {
        logger.info(`Diff no prompt: modo=${diffSection.mode}, ${formatDiffSizeKb(diffSection.totalBytes)}, ` +
    ...
    const agentResult = diffStats.fileCount > 0
        ? config.parallelChunks > 1
            ? await runParallelReview(config, {
                workItemContext,
                prDescriptionContext,
                existingReviewContext: reviewContext.contextForLlm,
                rulesContext: rulesMap.contextForPrompt,
            ...
            : await runCodeReviewAgent(config, {
                workItemContext,
                prDescriptionContext,
                existingReviewContext: reviewContext.contextForLlm,
                rulesContext: rulesMap.contextForPrompt,
    ...
    for (const review of parsed.reviews) {
        if (review.score != null) {
            logger.debug(`  [${review.severity}] score=${review.score} action=${review.developerAction} ${review.fileName}:${review.lineNumber}`);
        }
    }
    ...
    const wouldPostReviewsPre = getNewReviewsFromPlan(getCodeReviewPostingPlan(parsed).reviewsJson, reviewContext.existingKeys, config.scoreMin);
    ...
    if (escalate) {
        ...
        logger.warn(`Escalonamento: rodada ${currentRound} > limite ${config.maxRounds}. ` +
    ...
    const wouldPostReviews = escalate
        ? getNewReviewsFromPlan(getCodeReviewPostingPlan(effectiveParsed).reviewsJson, reviewContext.existingKeys, config.scoreMin)
        : wouldPostReviewsPre;
    ...
    if (isDryRunOrNoContext) {
        ...
        if (hasContext) {
            const simulated = simulateThreadResolution(reviewContext.fileReviewThreads, pendingThreads, parsed.resolvedThreads);
            resolvedCount = simulated.resolvedCount;
            pendingThreads = simulated.pendingThreads;
            if (resolvedCount > 0) {
                logger.info(`[dry-run] Simulando resolução/recuperação: ${resolvedCount} thread(s) removida(s) do gate pendente.`);
            }
            if (currentRound > 0 && hasOpenIssues) {
                logger.info(`[dry-run] Rodada ${currentRound}${config.maxRounds > 0 ? `/${config.maxRounds}` : ''}` +
        ...
        resolvedCount = await provider.resolvePullRequestReviewThreads(config.botTag, reviewContext.fileReviewThreads, parsed.resolvedThreads, (msg) => logger.info(msg));
        logger.info(`Resolved ${resolvedCount} active thread(s).`);
        if (resolvedCount > 0) {
            const afterResolve = await provider.getPullRequestReviewContext(config.botTag, (msg) => logger.debug(msg));
        ...
        const postedThreads = await provider.setPullRequestComments(config.botTag, postingPlan.reviewsJson, reviewContext.existingKeys, (msg) => logger.info(msg));
        postedReviews = postedThreads.map((item) => item.review);
        const refreshedContext = await provider.getPullRequestReviewContext(config.botTag, (msg) => logger.debug(msg));
        ...
        if (summaryPlan.postSummary) {
            ...
            await provider.setPullRequestReviewSummary(config.botTag, summaryPlan.reviewSummary, refreshedContext.allThreads, (msg) => logger.info(msg));
        }
        else {
            logger.info(`Skipping final review summary (${gatePendingAfterPost.length} active/pending bot thread(s) remain).`);
        }
        ...
        if (currentRound > 0 && (hasOpenIssues || escalate)) {
            ...
            await provider.persistRoundState(config.botTag, { currentRound, maxRounds: config.maxRounds, escalate, suppressedCount }, priorRoundState, (msg) => logger.info(msg));
        }
    }
    ...
    console.log(formatGateSummary(gate, agentResult.sessionId, agentResult.runId, isDryRunOrNoContext, agentResult.metrics));
