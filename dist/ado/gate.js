...
export function formatGateSummary(gate, agentId, runId, dryRun, metrics) {
    ...
    if (hasEngineMetrics(metrics)) {
        ...
        lines.push(`│ Tokens input:        ${formatTokenCount(input)}`, `│ Tokens output:       ${formatTokenCount(output)}`, `│ Tokens total:        ${formatTokenCount(total)}`);
    ...
    if (gate.pendingThreads.length > 0) {
        ...
        for (const thread of gate.pendingThreads) {
            const location = thread.filePath != null
                ...
                : '(thread geral)';
            lines.push(`  - #${thread.threadId} [${thread.status}] ${location} | autor: ${thread.author}`);
