...
export function buildReviewSummaryMarkdown(gate, reviews, dryRun, metrics) {
    const lines = [];
 ...
 */
export function emitPipelineReviewOutput(gate, reviews, dryRun, metrics, log = console.log) {
    if (!isAzurePipeline()) {
        return;
    }
    for (const review of reviews) {
