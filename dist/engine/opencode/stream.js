...
import { EMPTY_ASSISTANT_TEXT_CONTINUATION, formatIncompleteAssistantDiagnostics, mergeAssistantMetrics, resolveIncompleteAssistantTurn, } from './assistant-text.js';
...
async function sendSessionPrompt(client, sessionId, directory, agentName, prompt, modelSelection, logger, signal, variant) {
...
export async function runOpencodeStream(config, options, logger) {
    ...
    try {
        ...
        let response;
        try {
            response = await withAbortSignal(abortController.signal, sendSessionPrompt(client, activeSessionId, directory, agentName, options.prompt, modelSelection, logger, abortController.signal, config.variant));
            logToolParts(response.parts, logger);
            const firstMetrics = assistantMessageToMetrics(response.info);
            const resolved = await resolveIncompleteAssistantTurn({ info: response.info, parts: response.parts }, async () => {
                clearTimeout(timeoutHandle);
                timeoutHandle = setTimeout(() => abortController.abort(), timeoutMs);
                const followUp = await withAbortSignal(abortController.signal, sendSessionPrompt(client, activeSessionId, directory, agentName, EMPTY_ASSISTANT_TEXT_CONTINUATION, modelSelection, logger, abortController.signal, config.variant));
                ...
                logger.warn(`OpenCode: resposta incompleta sem textPart utilizável (${diagnostics}). ` +
