...
export function permissionReplyForType(permissionType) {
...
export function isGlobalEvent(raw) {
...
function syncLegacyType(syncType) {
...
export function normalizeSyncEvent(syncType, data) {
...
function isSyncEnvelope(raw) {
    const payload = raw.payload;
    return typeof payload === 'object' && payload !== null && payload.type === 'sync';
}
/**
 * Aceita envelope legado (`properties`) ou sync (`syncEvent`).
 * Retorna `'skip'` para sync válido sem handler (ex.: `session.updated.1`).
 */
export function parseGlobalEvent(raw) {
    if (isGlobalEvent(raw))
        return raw;
    if (!raw || typeof raw !== 'object')
        return undefined;
    const envelope = raw;
    if (envelope.directory !== undefined && typeof envelope.directory !== 'string')
        return undefined;
    if (!isSyncEnvelope(envelope))
        return undefined;
    const payload = envelope.payload;
    const syncEvent = payload.syncEvent;
    if (!syncEvent || typeof syncEvent !== 'object' || syncEvent === null)
        return undefined;
    const se = syncEvent;
    ...
    if (!data || typeof data !== 'object' || data === null)
        return undefined;
    const normalizedPayload = normalizeSyncEvent(syncType, data);
...
function parseStreamFlag(value, defaultValue) {
...
function resolveStreamReasoning(options) {
    return options.streamReasoning ?? parseStreamFlag(env.opencodeStreamReasoning(), true);
}
async function autoReplyPermission(client, sessionId, directory, permission, logger) {
    const response = permissionReplyForType(permission.type);
    logger.info(`[permission] ${permission.type} — auto-${response === 'reject' ? 'reject' : response} (${permission.title})`);
...
function handleEvent(event, options, streamState) {
    if (event.directory && !directoriesMatch(event.directory, options.directory))
        return;
    const payload = event.payload;
    if (!payload)
        return;
    if (!belongsToSession(payload, options.sessionId))
        return;
    switch (payload.type) {
        case 'session.status':
        ...
        case 'session.idle':
            ...
            break;
        case 'permission.updated':
            void autoReplyPermission(options.client, options.sessionId, options.directory, payload.properties, options.logger).catch((error) => {
        ...
        case 'message.part.updated': {
            ...
            if (part.type === 'tool') {
                ...
                if (streamState.toolStatuses.get(key) === status)
                    return;
            ...
            if (part.type === 'reasoning' && !streamReasoning)
                return;
            if (part.type === 'text' && !options.verbose)
                return;
...
function writeStreamDelta(prefix, stream, delta, state) {
...
function resolveStreamInactivityMs(options) {
...
export async function checkOpencodeStreamHealth(options, signal) {
...
function failStreamOnInactivity(options, streamAbort, inactivityMs) {
    const error = new OpencodeStreamInactivityError(`OpenCode server não responde após ${Math.round(inactivityMs / 1000)}s sem eventos no stream SSE`);
    options.fatalAbort?.abort(error);
    streamAbort.abort(error);
}
function createInactivityWatchdog(options, streamAbort, streamSignal) {
    ...
    const runCheck = async () => {
        if (streamSignal.aborted || checkRunning)
            return;
        checkRunning = true;
        try {
            ...
            if (streamSignal.aborted)
                return;
            if (healthy) {
                options.logger.info(`OpenCode: sem eventos há ${Math.round(inactivityMs / 1000)}s, servidor OK — aguardando...`);
                schedule();
                return;
            }
            options.logger.error(`OpenCode: sem eventos há ${Math.round(inactivityMs / 1000)}s e session.get falhou — encerrando stream`);
...
export async function consumeOpencodeEventStream(options) {
    const streamState = {
        lastStream: 'other',
...
export function startOpencodeEventStream(options) {
    ...
    const task = consumeOpencodeEventStream({
        ...
        if (linkedSignal.aborted)
            return;
        options.logger.warn(`OpenCode event stream encerrado: ${error instanceof Error ? error.message : String(error)}`);
    });
    return {
        stop() {
            controller.abort();
