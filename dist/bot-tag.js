...
const AGENTIC_REVIEWER_TAG_PREFIXES = [BOT_TAG_PREFIX, LEGACY_BOT_TAG_PREFIX];
/** Tag publicada nos comentários: `Agentic Code Reviewer {engine}`. */
export function buildBotTag(engine) {
...
export function extractAgenticBotTagLine(content) {
    ...
    if (match)
        return match[0];
    if (firstLine.startsWith(LEGACY_BOT_TAG_PREFIX))
