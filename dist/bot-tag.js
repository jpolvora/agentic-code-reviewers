...
export function buildBotTag(engine, version) {
    const ver = version?.trim();
    return ver ? `${BOT_TAG_PREFIX} v${ver} (${engine})` : `${BOT_TAG_PREFIX} (${engine})`;
}
function firstLineLooksLikeRunnerTag(firstLine) {
    return (/^agentic-code-reviewers(?:\s+v\S+)?(?:\s+\([^)]+\))?$/.test(firstLine) ||
        /^Agentic Code Reviewer(?: \S+)?$/.test(firstLine) ||
        firstLine === LEGACY_BOT_TAG_PREFIX);
}
function firstLineTagMatch(firstLine) {
...
export function extractAgenticBotTagLine(content) {
    if (!isAgenticReviewerComment(content))
        return null;
    const firstLine = content.split(/\r?\n/)[0]?.trim() ?? '';
    return firstLineTagMatch(firstLine);
}
/** Remove tags do runner (atual e legadas) para comparação/dedup de conteúdo. */
export function stripAgenticBotTags(content) {
    let text = content;
...
export function commentHasRoundStateMarker(content) {
    return isAgenticReviewerComment(content) && content.includes(ROUND_STATE_MARKER);
}
//# sourceMappingURL=bot-tag.js.map