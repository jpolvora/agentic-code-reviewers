...
function commentPrefix(comment, length = 80) {
...
export function mergeReviews(chunks) {
    const merged = [];
    for (const reviews of chunks) {
        for (const review of reviews) {
            const exactKey = reviewDedupKey(review.fileName, review.lineNumber);
            const exactIdx = merged.findIndex((r) => reviewDedupKey(r.fileName, r.lineNumber) === exactKey);
            if (exactIdx >= 0) {
...
export function mergeCodeReviewResponses(responses) {
    ...
    const reviewSummary = reviews.length === 0 && summaries.length > 0
        ? summaries.join('\n\n---\n\n')
        : '';
    return { reviews, resolvedThreads, reviewSummary };
}
//# sourceMappingURL=merge-reviews.js.map