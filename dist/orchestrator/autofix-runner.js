...
export function validateReplacements(replacements) {
    for (const rep of replacements) {
        if (!Number.isInteger(rep.startLine) ||
...
export function assertNonOverlapping(replacements) {
    const sorted = [...replacements].sort((a, b) => a.startLine - b.startLine);
    for (let i = 1; i < sorted.length; i++) {
        if (sorted[i].startLine <= sorted[i - 1].endLine) {
            throw new Error(`Replacements sobrepostos: ${sorted[i - 1].startLine}-${sorted[i - 1].endLine} e ${sorted[i].startLine}-${sorted[i].endLine}`);
        }
    }
}
export function applyReplacements(content, replacements) {
    ...
    for (const rep of sorted) {
        ...
        if (startIdx < 0 || endIdx >= lines.length || startIdx > endIdx) {
            throw new Error(`Substituição fora dos limites: linhas ${rep.startLine}-${rep.endLine} em arquivo de ${lines.length} linhas`);
        }
...
export function computeUpdatedLineNumber(originalLine, replacements) {
    const sorted = [...replacements].sort((a, b) => a.startLine - b.startLine);
    let mapped = originalLine;
    for (const rep of sorted) {
        if (originalLine < rep.startLine)
            break;
        if (originalLine <= rep.endLine) {
            const offset = originalLine - rep.startLine;
            const repStartMapped = rep.startLine +
                sorted
                    .filter((r) => r.startLine < rep.startLine)
                    .reduce((acc, r) => acc + r.replacementContent.split(/\r?\n/).length - (r.endLine - r.startLine + 1), 0);
...
export function isThreadLineModified(fileContent, updatedContent, threadLineNumber, replacements) {
    const inRange = replacements.some((rep) => threadLineNumber >= rep.startLine && threadLineNumber <= rep.endLine);
    if (!inRange)
        return false;
...
function buildResolvedItemsFromAgent(threads, parsed, dryRun) {
    const items = [];
...
async function tryRecoverPendingPush(config, reviewContext, provider, logger) {
    ...
    if (!buildOk) {
        throw new Error('Recovery dual-engine: build falhou — push do commit local pendente abortado.');
    }
    logger.section('Recovery: publicando commit pendente do engine anterior');
    const pushed = await pushAutoFixChanges(config, logger);
    if (!pushed) {
        throw new Error('Recovery dual-engine: falha ao publicar commit local pendente (threads já resolvidas por engine anterior).');
    }
    const subject = getHeadCommitSubject(config.repoRoot);
    const threadIds = parseAutoFixCommitThreadIds(subject);
    if (threadIds !== null) {
        const summary = buildRecoverySummary(getHeadCommitChangedFiles(config.repoRoot), threadIds);
        await postAutoFixSummary(config, reviewContext, provider, summary, logger);
    }
}
export function parseAutoFixCommitThreadIds(subject) {
...
function getHeadCommitSubject(repoRoot) {
    return execSync('git log -1 --format=%s', { cwd: repoRoot, encoding: 'utf8' }).trim();
}
function getHeadCommitChangedFiles(repoRoot) {
...
function formatCommonAutoFixSections(modifiedFiles) {
    const lines = [];
...
export function buildRecoverySummary(modifiedFiles, threadIds) {
    ...
    lines.push('');
    lines.push('> Published by the dual-engine recovery path after a prior engine committed and resolved threads but failed to push.');
    lines.push('');
    lines.push(`The auto-fix workflow successfully applied **${fixCount} fix(es)** across **${modifiedFiles.length} file(s)**.`);
    lines.push('');
...
async function postAutoFixSummary(config, reviewContext, provider, summary, logger) {
    ...
    if (!posted) {
        logger.warn('Auto-fix push/resolução concluídos, mas falha ao publicar sumário na PR — push já foi bem-sucedido.');
    }
    else {
        logger.info('Sumário do auto-fix publicado na PR.');
    }
}
export function getAutoFixThreads(reviewContext) {
    return reviewContext.fileReviewThreads;
}
function buildThreadUrl(config, thread) {
...
function buildAutoFixSummary(config, resolvedItems, activeThreads, modifiedFiles) {
    const resolvedWithThreads = resolvedItems
        .map((item) => {
        const thread = activeThreads.find((t) => String(t.threadId) === String(item.threadId));
        return { item, thread };
    })
    ...
    lines.push('## Auto-Fix Summary');
    lines.push('');
    lines.push(`The auto-fix workflow successfully applied **${resolvedWithThreads.length} fix(es)** across **${modifiedFiles.length} file(s)**.`);
    lines.push('');
    if (resolvedWithThreads.length > 0) {
        ...
        for (const { item, thread } of resolvedWithThreads) {
            ...
            lines.push(`| ${idx} | \`${file}\` | ${thread.lineNumber} | [discussion](${url}) | ${resolution} |`);
        }
        lines.push('');
    }
    lines.push(...formatCommonAutoFixSections(modifiedFiles));
...
function normalizeAutoFixSummaryBody(text) {
    let body = stripAgenticBotTags(text);
    body = body.replace(AUTO_FIX_SUMMARY_MARKER, '');
    return body.replace(/\s+/g, ' ').trim();
}
export function testAutoFixSummaryAlreadyPosted(reviewContext, _botTag, summaryText) {
    ...
    for (const thread of threads.value) {
        ...
        for (const comment of thread.comments) {
            if (comment.isDeleted || !isAgenticReviewerComment(comment.content))
                continue;
            if (!comment.content.includes(AUTO_FIX_SUMMARY_MARKER))
                continue;
...
export async function runAutoFixFlow(config, reviewContext, provider, engine, logger) {
    ...
    for (const filePath of filePaths) {
        ...
        const threadListText = threads
            .map((t) => `### Thread ${t.threadId} (linha ${t.lineNumber})\n${t.description || t.summary}`)
            .join('\n\n');
...
`;
        ...
        try {
            ...
            const verifiedResolvedItems = localResolvedItems.filter((item) => {
                const thread = threads.find((t) => (Number.isNaN(Number(item.threadId)) ? item.threadId : Number(item.threadId)) ===
                    (Number.isNaN(Number(t.threadId)) ? t.threadId : Number(t.threadId)));
                if (!thread)
                    return false;
                if (!isThreadLineModified(fileContent, updatedContent, thread.lineNumber, parsed.replacements)) {
                    logger.warn(`Thread ${thread.threadId} declarada resolvida mas linha ${thread.lineNumber} não foi alterada — ignorada.`);
                    return false;
                }
                return true;
            });
            if (verifiedResolvedItems.length === 0) {
                logger.warn(`Arquivo ${filePath} alterado mas nenhuma thread confirmada após verificação determinística — correção descartada.`);
                continue;
            }
            if (config.dryRun) {
    ...
    const buildOk = await runAutoFixBuild(config, logger);
    if (!buildOk) {
        throw new Error('Gate cooperativo: build falhou após commit local — resolução e push abortados.');
    }
    ...
    if (config.dryRun) {
        ...
        const resolvedCount = await provider.resolvePullRequestReviewThreads(config.botTag, activeThreads, resolvedItems, (msg) => logger.info(msg));
        logger.info(`Total de threads resolvidas: ${resolvedCount}/${resolvedItems.length}`);
        if (resolvedCount < resolvedItems.length) {
            logger.warn('Gate cooperativo: resolução incompleta — push abortado. Commit local preservado; ' +
    ...
    const pushed = await pushAutoFixChanges(config, logger);
    if (!pushed) {
        throw new Error('Gate cooperativo: push falhou após resolução de threads — PR e remoto podem estar inconsistentes.');
    }
    const summary = buildAutoFixSummary(config, resolvedItems, activeThreads, modifiedFiles);
    await postAutoFixSummary(config, reviewContext, provider, summary, logger);
}
//# sourceMappingURL=autofix-runner.js.map