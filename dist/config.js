...
import { DEFAULT_CURSOR_REVIEWER_MODEL, resolveCursorModelShape, } from './engine/cursor-sdk/model.js';
...
import { buildDefaultProtectedPatterns, DEFAULT_MAX_COMMENT_CHARS, } from './ado/safe-outputs.js';
...
function resolveReviewerModel(engine, cliModel) {
    const raw = resolveOptionalEnv(cliModel ?? env.model(), engine === 'opencode' ? DEFAULT_OPENCODE_MODEL : DEFAULT_MODEL);
...
export function parseDiffMaxBytes(value, fallback = DEFAULT_DIFF_MAX_BYTES) {
    if (typeof value === 'number') {
...
export function mergeIncludePatternsForSelfReview(base, reviewSelf) {
    if (!reviewSelf)
        return base;
    const merged = [...base];
...
export function loadConfig(argv = process.argv.slice(2)) {
    ...
    if (engine === 'cursor-sdk' && !cursorApiKey) {
        throw new Error(`${ENV.CURSOR_API_KEY} é obrigatório com engine cursor-sdk. Veja .env.example`);
    }
    ...
    if (!stackConfig) {
        ...
        try {
            if (stackConfig.name === 'Custom' && !customPromptVal) {
                throw new Error(`A stack "Custom" requer a definição do parâmetro --custom-prompt ou da variável de ambiente ${ENV.CUSTOM_PROMPT}.`);
            }
            if (customPromptVal) {
                if (stackConfig.name !== 'Custom') {
                    throw new Error(`--custom-prompt / ${ENV.CUSTOM_PROMPT} só é permitido com --stack=Custom.`);
                }
    ...
    if (autoFix && pullRequestId <= 0) {
        throw new Error('O modo auto-fix requer o ID de uma Pull Request (--pr-id ou AGENTIC_CODE_REVIEWERS_PR_ID).');
    }
    ...
    return {
        ...
        scoreMin: parseScoreMin(cli.scoreMin != null && Number.isFinite(cli.scoreMin)
            ? cli.scoreMin
            : env.scoreMin()),
        diffMaxBytes: parseDiffMaxBytes(cli.diffMaxBytes != null && Number.isFinite(cli.diffMaxBytes)
            ? cli.diffMaxBytes
            : env.diffMaxBytes(), DEFAULT_DIFF_MAX_BYTES),
        ...
        autoFix,
        autoFixBuildCommand: resolveAutoFixBuildCommand(repoRoot, env.autoFixBuildCommand()),
    };
}
