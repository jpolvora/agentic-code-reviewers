...
Hook runs `bash {skillsRoot}/ws-secrets-leak-review/scripts/pre-commit.sh` on staged files (which calls `bash {skillsRoot}/ws-secrets-leak-review/scripts/secrets_scanner.sh`). It **skips loudly** — printing why — when the skill or `rg` is missing. If the scanner is resolved and then exits non-zero (crash or unexpected error), the hook **blocks the commit**. Override: `git commit --no-verify`.
